...
class _aggregate(Function):
    ...
    def forward(ctx, A, X, C):
        ...
        E = encoding_ext.aggregate_forward(A, X, C)
        return E

    @staticmethod
    def backward(ctx, gradE):
        A, X, C = ctx.saved_variables
        gradA, gradX, gradC = encoding_ext.aggregate_backward(gradE, A, X, C)
...
class _scaled_l2(Function):
    ...
    def forward(ctx, X, C, S):
        SL = encoding_ext.scaled_l2_forward(X, C, S)
    ...
    def backward(ctx, gradSL):
        ...
        gradX, gradC, gradS = encoding_ext.scaled_l2_backward(gradSL, X, C, S, SL)

...
from ...ops import SyncBatchNorm

__all__ = ['DANet', 'get_danet']

class DANet(BaseNet):
    def __init__(self, nclass, backbone, aux=False, se_loss=False,
                 norm_layer=SyncBatchNorm, **kwargs):
        super(DANet, self).__init__(nclass, backbone, aux, se_loss,
                                     norm_layer=norm_layer, **kwargs)
        self.head = DANetHead(2048, self.nclass, norm_layer=norm_layer)
...
def get_danet(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(
            get_model_file('danet_%s_%s'%(backbone, acronyms[dataset]), root=root)))
    return model    